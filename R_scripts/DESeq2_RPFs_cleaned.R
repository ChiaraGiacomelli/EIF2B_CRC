...
sample_info <- data.frame(row.names = RPF_sample_names,
                          shRNA = factor(c(rep('NTC', 8), rep('2B1',8))),
...
write_csv(DEseq2_output, file = file.path(tables_dir, paste0("RPFs_", shRNA, "_NTCcorrected_DEseq2_apeglm_LFC_shrinkage.csv")))
...
write_csv(normalised_counts, file = file.path(tables_dir, paste0("RPFs_",shRNA, "_NTCcorrected_normalised_counts.csv")))

#plot PCA----
pcaData <- plotPCA(rld, intgroup=c("condition", "batch","shRNA"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))

pdf(file = paste0(plot_dir, shRNA, "_NTCcorrected_RPFs_PCA.pdf"), width = 8, height = 7.5)
...
pdf(file = paste0(plot_dir, shRNA, "_NTCcorrected_RPFs_batchCorrect_PCA.pdf"), width = 8, height = 7.5)
ggplot(pcaData, aes(PC1, PC2, color=shRNA, shape=condition, label = batch)) +
  geom_point(size=5) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
  ...
  theme(axis.title = element_text(size = 18),
        ...
        legend.title = element_text(size = 18),
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))+
  ggtitle(paste(shRNA, "RPFs batch corrected")) +
  geom_text_repel(aes(label=batch), colour = 'black',size = 6)
dev.off()
...
#you need to make sure this data frame is correct for your samples
sample_info <- data.frame(row.names = RPF_sample_names,
                          shRNA = factor(c(rep('NTC', 8), rep('2B4',8))),
                          condition = factor(c(rep("min", 4), rep(treatment, 4), rep("min", 4), rep(treatment, 4))),
                          batch = factor(c(rep(1:4, 4))))
...
#you need to make sure this data frame is correct for your samples
sample_info <- data.frame(row.names = RPF_sample_names,
                          shRNA = factor(c(rep('NTC', 8), rep('2B5',8))),
                          condition = factor(c(rep("min", 4), rep(treatment, 4), rep("min", 4), rep(treatment, 4))),
                          batch = factor(c(rep(1:4, 4))))
